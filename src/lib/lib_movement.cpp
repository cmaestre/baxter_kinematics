...
void done_callback(const actionlib::SimpleClientGoalState& state,
        ...
        //state.isDone()
        if(state.isDone()){
                eef_values.set_trajectory_done(true);
                ROS_WARN_STREAM("DONE CALLBACK : " << state.toString());
            }
        else{
                ROS_WARN("NOT DONE YET :(");
                eef_values.set_trajectory_done(false);
            }
    }
...
void feedback_callback(const control_msgs::FollowJointTrajectoryFeedbackConstPtr& feedback,
                       Kinematic_values& eef_values,
                       double full_time, double half_time){
        //eef_values.get_current_trajectory_speed() = feedback->actual.velocities;

        --------------actual velocities are empty vectors ------------------------------------
        ROS_INFO_STREAM("size of velocities in the feedback is: " << feedback->actual.velocities.size());
        eef_values.get_current_trajectory_speed().push_back(feedback->actual.velocities);
//        ROS_INFO_STREAM("size of velocities in the feedback is: " << eef_values.get_current_trajectory_speed().size());
        if(feedback->desired.time_from_start.toSec() >= half_time){
//                ROS_INFO_STREAM("Half time is : " << half_time
//                                << " desired now is : " << feedback->desired.time_from_start.toSec()
//                                << " and full time is : " << full_time);
                eef_values.set_execute_new_trajectory(true);
            }
    }
...
//            }
        ...
        //robot_state::RobotStatePtr start_state_second_trajectory = group->getCurrentState();
        //moveit::core::jointStateToRobotState(eef_values.get_joint_state(), *start_state_second_trajectory);
        //group->setStartState(*start_state_second_trajectory);

        //eef_values.get_baxter_mover_class()->group->setStartState(*eef_values.get_baxter_mover_class()->group->getCurrentState());
        moveit_msgs::RobotTrajectory robot_trajectory;
        double fraction = group->computeCartesianPath(waypoints, 0.01, 0.0, robot_trajectory);
        //double fraction = eef_values.get_baxter_mover_class()->group->computeCartesianPath(waypoints, 0.01, 0.0, robot_trajectory);
        ...
        while(fraction < 1.0 && trials < 50){ // if correct orientation is not feaseble, explore other ones
                ...
                fraction = group->computeCartesianPath(waypoints, 0.01, 0.0, robot_trajectory);
                //fraction = eef_values.get_baxter_mover_class()->group->computeCartesianPath(waypoints, 0.01, 0.0, robot_trajectory);
                pitch_counter += 1;
                trials += 1;
            }
...
//        ROS_INFO_STREAM("Robot Trajectory total time is : " << robot_trajectory.joint_trajectory.points.back().time_from_start.toSec()
//                        << " and its half time is : " << robot_trajectory.joint_trajectory.points.back().time_from_start.toSec()/2.0);

        double full_time = robot_trajectory.joint_trajectory.points.back().time_from_start.toSec();
        double half_time = full_time/2.0;

        control_msgs::FollowJointTrajectoryGoal goal;
        goal.trajectory = robot_trajectory.joint_trajectory;
        goal.goal_time_tolerance = ros::Duration(0);
        eef_values.set_trajectory_total_time(goal.trajectory.points.back().time_from_start.toSec());
        eef_values.set_trajectory_half_time(goal.trajectory.points.back().time_from_start.toSec()/2.0);

//        ROS_INFO_STREAM("Trajectory total time is : " << eef_values.get_trajectory_total_time()
//                        << " and its half time is : " << eef_values.get_trajectory_half_time());
        ROS_ERROR_STREAM("Trajectory size is : " << goal.trajectory.points.size());

        prev_obj_pos = false;
        if (feedback_data){
                //ROS_ERROR("I am in the one with feedback");
                ac.sendGoal(goal,
                            boost::bind(done_callback, _1, boost::ref(eef_values)),
                            boost::bind(active_callback),
                            boost::bind(feedback_callback, _1,
                                        boost::ref(eef_values),
                                        boost::ref(full_time),
                                        boost::ref(half_time)));
                while(!eef_values.get_execute_new_trajectory() && !ac.getState().isDone());
                if(eef_values.get_execute_new_trajectory()/*execute new trajectory*/){
                        ROS_WARN_STREAM("I am preempting the trajectory, total time is : " << goal.trajectory.points.back().time_from_start.toSec());
                        ac.cancelAllGoals();
                        ROS_ERROR("***********************************************************");
                        return 3; //trajectory is preempted
                    }
            }
        else
            ac.sendGoal(goal);
